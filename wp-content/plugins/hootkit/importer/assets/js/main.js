jQuery( function ( $ ) {
	...
	$( '.js-ocdi-import-data' ).on( 'click', function () {
		...
		if ( $(this).is('.hk-deactivated') ) { return; }
		var $acceptnotice = $('#ocdi__hk-acceptnotice'),
			$acceptinput = $('#hkocdiaccept'),
			accept = $acceptinput.val();
		$acceptnotice.slideUp();
		$acceptinput.css('border','1px solid #ddd');
		if ( accept.toLowerCase().trim() != 'accept' ) {
			$acceptnotice.slideDown();
			$acceptinput.css('border','1px solid #dc3232');
			var destin = $acceptinput.offset().top - 80; 
	...
	function ajaxCall( data ) {
		$.ajax({
			...
			beforeSend:  function() {
				...
				$( '.js-ocdi-import-data' ).addClass('hk-deactivated');
			}
		})
		.done( function( response ) {
			if ( 'undefined' !== typeof response.status && 'newAJAX' === response.status ) {
			...
			else if ( 'undefined' !== typeof response.status && 'hkwidgetsAJAX' === response.status ) {
				var newData2 = new FormData();
				newData2.append( 'action', 'ocdi_import_hkwidget_data' );
				newData2.append( 'security', ocdi.ajax_nonce );
				ajaxCall( newData2 );
			}
				...
				if ( data.get('hk_flag') )
					newData.append( 'hk_flag', data.get('hk_flag') );
				...
				$( '.js-ocdi-import-data' ).removeClass('hk-deactivated');

				// Trigger custom event, when HKOCDI import is complete.
