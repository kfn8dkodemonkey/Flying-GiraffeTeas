...
function hoot_get_attr( $slug, $context = '', $attr = '' ) {
	...
	/* Define variables */
	$out = '';
	$customclass = ( !is_array( $attr ) ) ? $attr : '';
	$attr = ( !is_array( $attr ) ) ? array() : $attr;
	$attr['customclass'] = ( !empty( $customclass ) ) ? $customclass : '';

	/* Build attrs */
	// $slugger = str_replace( "-", "_", $slug );
	// $attr    = wp_parse_args( $attr, apply_filters( "hoot_attr_{$slugger}", array(), $context, $attr ) );
	$attr = apply_filters( "hoot_attr_{$slug}", $attr, $context );
	if ( !isset( $attr['class'] ) )
		$attr['class'] = $slug;

	/* Add custom Classes if any */
	if ( !empty( $attr['customclass'] ) && is_string( $attr['customclass'] ) )
