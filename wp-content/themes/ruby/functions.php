...
/* Add new product data type */
add_action('init', 'register_market_product_type');

function register_market_product_type(){
	class WC_Product_Market extends WC_Product{
		public function __construct($product){
			$this->product_type = 'market';
		parent::__construct($product);
		}
	}
}

add_filter('product_type_selector', 'add_market_product_type');

function add_market_product_type($types){
	$types['market'] = __('Markets', 'mk_product');
	
	return $types;
}

add_filter( 'woocommerce_product_data_tabs', 'market_product_tab' );
 
function market_product_tab( $tabs) {
         
    $tabs['market'] = array(
      'label'    => __( 'Market Product', 'mk_product' ),
      'target' => 'market_product_options',
      'class'  => 'show_if_market_product',
     );
    return $tabs;
}

add_action( 'woocommerce_product_data_panels', 'market_product_tab_product_tab_content' );
 
function market_product_tab_product_tab_content() {
 
 ?><div id='market_product_options' class='panel woocommerce_options_panel'><?php
 ?><div class='options_group'><?php
                 
    woocommerce_wp_text_input(
    array(
      'id' => 'martket_product_info',
      'label' => __( 'Market Product Spec', 'mk_product' ),
      'placeholder' => '',
      'desc_tip' => 'true',
      'description' => __( 'Enter Market product Info.', 'mk_product' ),
      'type' => 'text'
    )
    );
 ?></div>
...
add_action( 'woocommerce_process_product_meta', 'save_market_product_settings' );
     
function save_market_product_settings( $post_id ){
         
    $market_product_info = $_POST['market_product_info'];
         
    if( !empty( $market_product_info ) ) {
 
    update_post_meta( $post_id, 'market_product_info', esc_attr( $market_product_info ) );
    }
}


/* end new product data type */
